        ...
        <div>
            {_props.vals.map((e,i) => 
                <Elem 
                    ...
                    data={e} 
                    set={_props.setter}
                    onChange={ev => updateSelection(ev)}
                />)
            }<br/>   
        </div>    
    );
    ...
    const zakTypes = ["организация","ИП","физическое лицо"];
        ...
        <label for="pet-select">Choose a pet:</label>
            ...
            >
                ...
                {objects.map(e => (<option value={`${e}`}> {e} </option> ))}
            </select>
            <select
                value={dogTypeSel} 
                id="dogTypeSel" 
            ...
            >
                <option value="">--Please choose an option--</option>
                {dogProjData.map(e => (<option value={`${e}`}> {e} </option> ))}
            </select>
            <select
                value={yurlitsoSel} 
                id="yurlitsoSel" 
...
//  ROOT COMOPOSER -------------------------------------------
    ...
    const FormTab = _props => {
        return (zakazchik==="организация"
                    ? <FormOne data={_props.data} clientType={"Org"}/> 
                    : zakazchik==="ИП"
                        ? <FormTwo data={_props.data} clientType={"IP"}/>
                        : zakazchik==="физическое лицо"
                            ? <FormThree data={_props.data} clientType={"FL"}/> 
